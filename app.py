...
class YouthPolicyAPI:
    ...
    def search_policies(self, age: int = None, region: str = None, 
                       keyword: str = None, max_results: int = 3) -> List[Dict]:
...
# ============================
# 4. Polyglot-ko Inference API
# ============================

class PolyglotKoAPI:
    """✅ Polyglot-ko 5.8B - Polyglot-ko과 유사한 성능"""
    def __init__(self, model_name: str = "EleutherAI/polyglot-ko-5.8b", hf_token: str = None):
        self.api_url = f"https://api-inference.huggingface.co/models/{model_name}"
        self.headers = {}
        if hf_token:
            self.headers["Authorization"] = f"Bearer {hf_token}"
        
        print(f"✅ Polyglot-ko API 초기화: {model_name}")

    def generate_response(self, prompt: str, max_new_tokens: int = 120, max_retries: int = 2) -> str:
        """Inference API로 응답 생성"""
        payload = {
            "inputs": prompt,
            "parameters": {
...
class UnifiedPolicyChatbot:
    def __init__(self, youth_api_key: str, bizinfo_api_key: str, alioplus_api_key: str, hf_token: str = None):
        ...
        self.polyglot = PolyglotKoAPI(hf_token=hf_token)
        print("✅ 챗봇 초기화 완료! (Polyglot-ko)")
    ...
    def extract_user_info(self, message: str) -> Dict:
        ...
        # 키워드 추출
        keywords = ['창업', '취업', '주거', '자격증', '대출', '교육', 'R&D', '일자리', '면접']
        for kw in keywords:
            if kw in message:
                info['keyword'] = kw
                break
        ...
        search_verbs = ['찾아', '검색', '뽑아', '추천', '보여', '알려']
    ...
    def chat(self, message: str) -> str:
        ...
        return self.polyglot.generate_response(prompt, max_new_tokens=120)
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("🚀 통합 정책 추천 챗봇 API 서버 시작 (Polyglot-ko)")
    print("="*60)
